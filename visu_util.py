...
def plot_pcd_img(filename, pcds, img, titles, suptitle='', sizes=None, cmap='Reds', zdir='y',
    ...
    for i in range(3):
        ...
        for j, (pcd, size) in enumerate(zip(pcds, sizes)):
            color = pcd[:, 0]
            ax = fig.add_subplot(3, len(pcds)+1, i * (len(pcds)+1) + j + 1, projection='3d')
            ...
            ax.set_ylim(ylim)
            ax.set_zlim(zlim)
        ax = fig.add_subplot(3, len(pcds)+1, (i+1) * (len(pcds)+1))
...
def plot_pcd_three_views(filename, pcds, titles, suptitle='', sizes=None, cmap='Reds', zdir='y',
    ...
    if sizes is None:
        sizes = [0.5 for i in range(len(pcds))]
    fig = plt.figure(figsize=(len(pcds)+1 * 3, 9))
    for i in range(3):
        elev = 30
        azim = -45 + 90 * i
        for j, (pcd, size) in enumerate(zip(pcds, sizes)):
